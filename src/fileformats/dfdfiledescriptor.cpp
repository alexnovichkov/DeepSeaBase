...
//    return;
    ...
    // работаем с файлом raw
    //1. копируем каналы из file в this
    foreach (int i, indexes) {
        Channel *ch = file->channel(i);
        ...
        RawChannel *rawSource = dynamic_cast<RawChannel *>(ch);
        RawChannel *rawDest = dynamic_cast<RawChannel *>(channels[0]);
